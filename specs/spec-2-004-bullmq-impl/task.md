...
## 1. 사전 준비 (Setup)
- [x] 브랜치 생성 (`git checkout -b spec-2-004/bullmq-impl`)
- [x] Node.js 의존성 추가 (`bullmq`, `@bull-board/api`, `@bull-board/express`, `express`, `@types/express` → `workers/node/package.json`, `pnpm install`)
- [x] Python 의존성 추가 (`rq`, `redis` → `workers/python/requirements.txt`, `api-server/python/requirements.txt`)

## 2. Producer 구현 (API Server)
- [x] `api-server/python/main.py` — `BullMQQueue` 클래스 구현 및 `POST /bullmq/orders` 엔드포인트 추가
  - `redis-py` 로 BullMQ Redis 키 구조(`bull:orders:*`)에 직접 Job enqueue
  - `rq` 로 `orders-python` 큐에도 동시 enqueue

## 3. Node.js Worker 구현 (BullMQ)
- [x] `workers/node/src/bullmq.worker.ts` 파일 생성
...
- [x] `workers/python/bullmq_worker.py` 파일 생성
...
- [x] `docker compose up postgres redis -d` 로 인프라 기동 확인
- [x] BullMQ Worker, RQ Worker 동시 실행 후 `POST /bullmq/orders` 5회 호출
- [x] DB에서 `mq_type='bullmq'` 레코드 조회 — inventory-group + payment-group 모두 확인
- [x] `SIMULATE_FAILURE=true` BullMQ Worker 실행 후 메시지 발행 → Redis에서 `atm:3` + stacktrace 3개 + failed 상태 확인

## 6. 리뷰 및 마무리 (Review & Wrap-up)
- [x] 개발된 내용이 정상 동작하는지 Agent 차원의 자체 로컬 테스트 및 검증 (필수)
- [x] 전체 기능 동작 확인 및 린팅(Linting) / 포맷팅(Formatting)
- [x] `walkthrough.md` 작성 (아키텍처 다이어그램, 검증 결과 포함)
