...
- [x] **구현**: `aiokafka`, `kafkajs` 패키지를 사용해 공통 인터페이스 맞춤 구현 (Topic: `orders`)
- [x] **검증**: 결제 그룹(`payment-group`)과 재고 그룹(`inventory-group`) 생성 시 메시지 병렬 소비 확인.
- [ ] **기술 결정**: `docs/tech/adr/` 에 패키지 매니저 및 ORM 선택 배경 기록 완료.

### 🛠️ Spec 2-002. 아키텍처 표준화 및 기록 (Standardization)
> **동작 목표**: 일관된 개발 환경과 기술 문서(ADR)를 통해 후속 MQ 구현의 기반을 다집니다.
- [ ] **구현**: `pnpm workspace` 및 `SQLModel` 공통 모델 적용.
- [ ] **자동화**: `Alembic` 도입을 통한 DB 마이그레이션 관리 (Phase 3 전 완료 계획).

### 🐇 Spec 2-003. RabbitMQ 구현 (RabbitMQ MVP)
> **동작 목표**: RabbitMQ의 라우팅과 "재시도 후 실패 처리(DLQ)"를 집중 검증합니다.
- [x] **구현**: `aio-pika`, `amqplib` 활용 및 수동(manual) `ack`/`nack` 제어 로직 적용
- [x] **검증**: 워커 로직에서 인위적으로 예외 처리를 발생시킨 뒤, RabbitMQ 관리자 UI에서 해당 메시지가 Queue에 쌓이거나 `dead-letter.queue`로 이동됨을 눈으로 확인.
...
- [x] RabbitMQ의 ack/nack/DLQ 흐름을 다이어그램으로 그릴 수 있다
